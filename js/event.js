...
function loadItems(){
   myUserRef .on("value", function(snapshot) {
       if(!started){
             ...
             document.getElementById('username').innerHTML = message.fullName;
...
function loopThroughItems(obj){
   ...
   document.getElementById('rem').innerHTML = obj.length;
    ...
    for(var c =0; c < obj.length; c++){
        array.push( new addEvent(obj[c].title,obj[c].start,obj[c].backgroundColor,
        obj[c].borderColor) );
          $('#calendar').fullCalendar( 'addEvent',  new addEvent(obj[c].title,obj[c].start,obj[c].backgroundColor,
        obj[c].borderColor) );
    $('#calendar').fullCalendar('renderEvent', new addEvent(obj[c].title,obj[c].start,obj[c].backgroundColor,
        obj[c].borderColor) , true);
    }
    }
